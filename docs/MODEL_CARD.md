...
| Limitação | Descrição |
| :--- | :--- |
| **Dataset sintético** | Treinado sobre o **Oral Cancer Top 30 Countries** (MIT License, ~160k registros) — de origem **sintética/gerada**. As distribuições e variáveis são estatisticamente independentes do target (ver [Relatório de Auditoria de Dados](file:///f:/FIAP/Tech%20Challenge%20-%2001/Aether%20Oncology/docs/dataset_audit_report.md) detalhando a independência). Nenhuma métrica transfere para dados clínicos. Bons números indicam **aderência ao gerador de dados**, não capacidade preditiva no mundo real. |
| **Ausência de sinal preditivo** | A auditoria estatística revela correlação e informação mútua nulas (<0.01) de todas as features com o target. O benchmark mostra que o MLP, Regressão Logística e Random Forest possuem performance idêntica ao acaso (ROC-AUC ≈ 0.50), confirmando que a tarefa preditiva se resume a ruído puro sob o dataset atual (ver detalhes no [Relatório de Auditoria de Dados](file:///f:/FIAP/Tech%20Challenge%20-%2001/Aether%20Oncology/docs/dataset_audit_report.md)). |
