    ...
    def analyze(
        ...
        if overall_risk == "HIGH":
            recommendations.append(
                "Encaminhamento imediato para avaliação com especialista (oncologista ou cirurgião de cabeça e pescoço)."
            )
            recommendations.append(
                "Realização de biópsia sob qualquer suspeita clínica de lesão persistente por mais de 2 semanas."
            )
        elif overall_risk == "MEDIUM":
            recommendations.append(
                "Acompanhamento odontológico/médico preventivo semestral."
            )
            recommendations.append(
                "Orientação ativa para cessação do uso de tabaco e/ou álcool."
            )
        else:
